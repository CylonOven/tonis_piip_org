...
DJANGO_DATABASE_NAME=tonis_piip_org
DJANGO_DATABASE_USER=tonis_piip_org
...
def defaults():
    ...
    env.code_dir = '/srv/tonis_piip_org'
        ...
        "letsencrypt": [
            {
                ...
                "filename": "letsencrypt.tonis_piip_org.conf",
        ...
        "nginx": [
            {
                ...
                "filename": "app.tonis_piip_org.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "app.tonis_piip_org.proxy_django.include",
                "filename": "app.tonis_piip_org.proxy_django.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "%(target)s.ssl",
                "filename": "ssl.tonis_piip_org.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "default_site": True,
                "pattern": "%(target)s.conf",
                "filename": "tonis_piip_org",
...
def test():
    ...
    defaults()
    env.django_site = 'tonis-piip-org.test.tonis.piip.org'
    env.hosts = ['test.tonis.piip.org']
...
def live():
    ...
    env.django_site = 'tonis.piip.org'
    env.hosts = ['tonis.piip.org']
...
def setup_server(id=None):
    ...
    django_env_file_path = env.code_dir + '/tonis_piip_org/django.env'
    ...
    # Create database
    sudo('echo "CREATE DATABASE tonis_piip_org; '
         '      CREATE USER tonis_piip_org WITH password \'{db_password}\'; '
         '      GRANT ALL PRIVILEGES ON DATABASE tonis_piip_org to tonis_piip_org;" '
    ...
    # Create log dir
    sudo('mkdir -p /var/log/tonis_piip_org/')
    ...
    with cd(env.code_dir):
        sudo('cp deploy/logrotate.conf /etc/logrotate.d/tonis_piip_org')
...
def add_secret_keys():
    ...
    remote_path = env.code_dir + '/tonis_piip_org/django.env'
...
def docker_compose_run(service, cmd='', name='tonis_piip_org_tmp'):
...
def ensure_docker_networks():
    ...
    ensure_docker_network_exists(
        'tonis_piip_org_default', [], internal=False)
    ensure_docker_network_exists(
        'tonis_piip_org_nginx', ['nginx'])
    ensure_docker_network_exists(
        'tonis_piip_org_postgres', ['postgres-{postgres_version}'.format(
...
def ensure_local_py_exists():
    ...
    django_local_settings = env.code_dir + '/tonis_piip_org/settings/local.py'
