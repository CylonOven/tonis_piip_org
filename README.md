...
Browser support is defined in the `tonis_piip_org/browserslist` file that is used for autoprefixing CSS.
...
|run psql                              |`make psql`                            |`docker-compose exec postgres psql --user tonis_piip_org --dbname tonis_piip_org` |
|generate docs                         |`make docs`                            |`docker-compose run --rm django sphinx-build ./docs ./docs/_build`          |

## Running commands on the server

    docker-compose -f docker-compose.production.yml run --rm --name tonis_piip_org_tmp django python manage.py <command>
...
- python: `tonis_piip_org/cover`
- javascript: `tonis_piip_org/coverage`
...
* [Create the bucket for media files](http://docs.aws.amazon.com/AmazonS3/latest/UG/CreatingaBucket.html):
  * Bucket name: tonis_piip_org-{ENV} where `ENV` is either `staging` or `production`.
